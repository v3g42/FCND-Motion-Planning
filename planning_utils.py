...
def get_min_north_east(data):
    # minimum and maximum north coordinates
    north_min = np.floor(np.min(data[:, 0] - data[:, 3]))

    # minimum and maximum east coordinates
    east_min = np.floor(np.min(data[:, 1] - data[:, 4]))
    return int(north_min), int(east_min)


def create_graph_probabilistic(data, drone_altitude, safety_distance, k=10, sample_size=300):
...
def create_graph_voronoi(data, drone_altitude, safety_distance):
    """
